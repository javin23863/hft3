...
def test_sharpe_and_sortino_distinguish_edge_from_noise() -> None:
    from research_pipeline.statistics import sharpe_ratio, sortino_ratio

    rng = _seed_rng(1)
    good = rng.normal(0.0008, 0.005, 500)
    noise = rng.normal(0.0, 0.005, 500)
    assert sharpe_ratio(good.tolist()) > sharpe_ratio(noise.tolist())
    ...
    assert sortino_ratio(good.tolist()) >= sharpe_ratio(good.tolist())
    assert sortino_ratio(noise.tolist()) >= 0.0 or math.isfinite(sortino_ratio(noise.tolist()))
...
def test_psr_higher_for_genuine_edge() -> None:
    ...
    assert probabilistic_sharpe_ratio(good.tolist()) > 0.5
    assert probabilistic_sharpe_ratio(bad.tolist()) < 0.5


def test_dsr_deflates_with_trial_count() -> None:
    from research_pipeline.statistics import deflated_sharpe_ratio

    rng = _seed_rng(3)
    returns = rng.normal(0.0005, 0.004, 400)
    single = deflated_sharpe_ratio(returns.tolist(), num_trials=1)
    many = deflated_sharpe_ratio(returns.tolist(), num_trials=100)
...
def test_min_trl_finite_for_edge_infinite_for_no_edge() -> None:
    from research_pipeline.statistics import minimum_track_record_length

    rng = _seed_rng(4)
    good = rng.normal(0.0008, 0.005, 400)
    bad = rng.normal(-0.0008, 0.005, 400)
    assert math.isfinite(minimum_track_record_length(good.tolist()))
    assert minimum_track_record_length(bad.tolist()) == float("inf")


def test_risk_metrics_cvar_tail_ratio_skew_kurtosis() -> None:
    from research_pipeline.statistics import cvar, tail_ratio, skewness, kurtosis, max_drawdown
    ...
    assert isinstance(skewness(returns.tolist()), float)
    assert isinstance(kurtosis(returns.tolist()), float)
    assert max_drawdown(returns.tolist()) >= 0.0


def test_statistics_empty_and_insufficient_inputs_are_safe() -> None:
    from research_pipeline.statistics import summary_metrics, probabilistic_sharpe_ratio
...
def test_cost_breakdown_subtracts_all_cost_components() -> None:
    from research_pipeline.cost_model import apply_costs, micro_standard_cost_config

    trades = [{"side": "buy", "qty": 2, "fill_price": 4500.0, "notional": 9000.0} for _ in range(10)]
    cfg = micro_standard_cost_config()
    b = apply_costs(5000.0, trades, cfg)
...
def test_cost_adjusted_returns_reduce_gross_for_costs() -> None:
    ...
    assert float(net.sum()) < float(gross.sum())


def test_empty_trades_yield_gross_equals_net() -> None:
    from research_pipeline.cost_model import apply_costs, micro_standard_cost_config

    b = apply_costs(100.0, [], micro_standard_cost_config())
