"""Live broker adapter stub (explicit LIVE mode only; not wired to gateway hot path)."""
...
class LiveBrokerAdapter:
    source_adapter = "LiveBrokerAdapter"

    def __init__(self, run_id: str = "live") -> None:
        self._run_id = run_id
        self._events: List[OrderEvent] = []
        self._pending: List[OrderEvent] = []

    def submit_order(self, order_intent: OrderIntent) -> OrderEvent:
        safety.record_live_broker_call()
        ...
        ev = OrderEvent(
            ...
            rejection_reason="live_adapter_stub_not_wired",
