...
# Anchor-relative offsets in seconds (filtration-safe: state at T+offset uses events <= T+offset)
SNAPSHOT_OFFSETS_SEC: Tuple[int, ...] = (
    -300, -120, -60, -30, -10, -5, -1, 0, 1, 5, 10, 30, 60, 120, 300,
)
...
def _tick_size_for_symbol(research_symbol: str, repo_root: Path) -> float:
    ...
    if not _TICK_SIZE_CACHE:
        hot_path = repo_root / "workbench" / "config" / "hot_memory_universe.yaml"
        if hot_path.is_file():
            import yaml
...
def build_l3_event_tensor(
    ...
    *,
    ...
    csv_path = repo_root / "data_system" / "config" / "events.csv"
