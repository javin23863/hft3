#!/usr/bin/env python3
"""Canonical entry: dispatch to v2 paid-screen orchestrator (long-lived workers)."""
        ...
        if arg == _RETIRED_V1_FLAG:
            if i + 1 >= len(argv):
                print(
                    ...
                    "v1 orchestrator retired — use run_paid_screen.py (v2 only)",
                    file=sys.stderr,
                )
                raise SystemExit(2)
            mode = argv[i + 1]
            ...
            print(
                ...
                "run_paid_screen.py always uses v2",
                file=sys.stderr,
            )
            i += 2
            continue
...
def main(argv: list[str] | None = None) -> int:
    ...
    return subprocess.call(
        [sys.executable, str(_REPO / "scripts" / "run_vectorbt_paid_screen_v2.py")] + tail
    )
