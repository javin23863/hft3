...
if (Test-Path "C:\rtrader_smb.env") {
    Get-Content "C:\rtrader_smb.env" | ForEach-Object {
...
if ($viIso) {
    ...
    $viosInf = Get-ChildItem -Path $viRoot -Recurse -Filter 'vioscsi.inf' -ErrorAction SilentlyContinue | Select-Object -First 1
    if ($netInf) { pnputil /add-driver $netInf.FullName /install | Out-Null }
    if ($viosInf) { pnputil /add-driver $viosInf.FullName /install | Out-Null }
}

$shareRoot = "\\$SmbHost\rtrader_watch"
net use R: $shareRoot /user:$SmbUser $SmbPass /persistent:yes 2>$null

# Install R|Trader from portable zip on share
$zip = "R:\rithmic_portable.zip"
...
# Junction Documents\Rithmic -> R:\
$docs = "$env:USERPROFILE\Documents\Rithmic"
if (-not (Test-Path $docs)) {
    cmd /c mklink /J "$docs" "R:\"
}
...
Write-Output "Guest setup complete: $($exe.FullName) -> logs on R:\"
