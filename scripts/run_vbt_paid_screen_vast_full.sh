...
# v2 env knobs: VBT_BATCH_TIMEOUT_SECONDS, VBT_MAX_UNITS_PER_BATCH, VBT_CACHE_MEMORY_LIMIT_MB, VBT_CACHE_MAX_ENTRIES, VBT_MAX_BATCHES_BEFORE_RECYCLE, VBT_RESUME=1
...
if [[ "${VBT_WRITE_DECLARATION_TEMPLATE:-0}" == "1" || "${VBT_WRITE_DECLARATION_TEMPLATE:-0}" == "true" ]]; then
  ...
  python3 - "$DECL_FILE" "$UNIT_COUNT" "$NPROC" "$WORKERS" "$UNITS_SOURCE_DESC" "$GIT_HEAD" "$EVENTS_CSV_HASH" "$LAKE_MANIFEST_HASH" "$STALL_MINUTES" "$BATCH_TIMEOUT_SECONDS" "$MAX_UNITS_PER_BATCH" "$DECL_ABORT_ON_FAILED_UNITS" <<'PY'
import json
import os
import sys
from pathlib import Path

decl_path, unit_count, host_vcpu, workers, units_source, git_head, events_hash, lake_hash, stall_minutes, batch_timeout_seconds, max_units_per_batch, abort_on_failed_units = sys.argv[1:13]
payload = {
...
python3 - "$DECL_FILE" "$UNIT_COUNT" "$NPROC" "$WORKERS" "$UNITS_SOURCE_DESC" "$GIT_HEAD" "$EVENTS_CSV_HASH" "$LAKE_MANIFEST_HASH" "$STALL_MINUTES" "$BATCH_TIMEOUT_SECONDS" "$MAX_UNITS_PER_BATCH" <<'PY'
import json
import sys

decl_path, unit_count, host_vcpu, workers, units_source, git_head, events_hash, lake_hash, stall_minutes, batch_timeout_seconds, max_units_per_batch = sys.argv[1:12]
