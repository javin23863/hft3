...
def load_paper_shadow_config(repo_root: Path) -> dict[str, Any]:
    """Load options-lane paper shadow config from walk_forward.yaml options_paper_shadow block.

    Defaults: 60 days, paper account.
    """
    wf = load_walk_forward_config(repo_root)
    block = wf.get("options_paper_shadow") or {}
    return {
        "days": int(block.get("days", 60)),
        "account_type": str(block.get("account_type", "PAPER")),
        "lane": str(block.get("lane", "options")),
    }
...
def _run_options_lane_campaign(
    repo_root: Path,
    model_id: str,
    symbol: str,
    artifact_dir: Path,
    ...
    *,
    ...
    """Options lane dispatcher: only options/parity models are supported.

    Stock-type (EQUITY_/LOW_FLOAT_) campaigns moved to the hft3-equities-lane repo.
    """
    ...
    raise ValueError(
        f"model '{model_id}' is not an options/parity model; "
        "stock-type equities campaigns moved to the hft3-equities-lane repo"
    )
...
def run_campaign(
    ...
    *,
    ...
    if binding.get("campaign_mode") == "options_lane":
        ...
        return _run_options_lane_campaign(
            repo_root,
            model_id,
            symbol,
            artifact_dir,
