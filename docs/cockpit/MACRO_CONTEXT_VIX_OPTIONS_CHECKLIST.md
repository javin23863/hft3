...
| `apps/cockpit/backend/aggregate/options.py` | Add first-class read-only Options zone | `specs/OPTIONS_LANE.md`; Vault `decisions/2026-06-12 Options-lane build decisions (slices 1-7).md`; Vault `sessions/2026-06-13 Options backfill, study verdicts, cockpit integration.md` | CME options are research/backtest only in Phases 0-1; live/shadow arm is blocked while the options defect ledger is non-empty; data readiness already lives in data_doctor options checks. | Reuse existing System options readiness and defect-ledger functions; do not add a new pipeline or live controls. | Backend test asserts `/api/options` and `ZONES["options"]` expose independent options lane state, data readiness, defects, and research-only status. | implemented; focused backend tests and full cockpit pytest passed 2026-06-14 |
...
| `apps/cockpit/frontend/src/types.ts`; `apps/cockpit/frontend/src/views/OptionsView.tsx`; `apps/cockpit/frontend/src/App.tsx`; `apps/cockpit/frontend/src/components/Sidebar.tsx` | Add first-class Options cockpit page | Same authorities as `apps/cockpit/backend/aggregate/options.py` | Cockpit must show options data readiness, open defects, research-only status, and no live/paper controls until gates clear. | Add `/options` page with readiness/defect cards sourced only from `/api/options`; no launch or order controls. | Frontend build and browser check. | implemented; frontend build passed 2026-06-14 |
