...
Implementation rows for Pass 2 / Pass 3 start:

| touched_file | feature_or_rule | authority | source_claim | implementation_consequence | tests_or_artifacts | review_status |
|---|---|---|---|---|---|---|
| `apps/cockpit/backend/loaders.py` | Preserve Stage A VIX coverage fields in compact cell projection | `packages/research_pipeline/src/stage_a_screen.py`; `tests/test_stage_a/test_stage_a_outputs.py`; `docs/workbench/HOT_MEMORY_UNIVERSE.md` | Stage A cells carry `n_events_with_vix` / VIX coverage; VIX/VVIX are sensors/context, not executable instruments. | The cockpit loader must not drop VIX coverage or silently convert absent VIX into zero-alpha claims. | Backend test verifies nested and flat VIX coverage projects into Models zone. | implemented; targeted pytest passed 2026-06-14 |
| `apps/cockpit/backend/aggregate/models.py` | Aggregate and expose VIX coverage in Models zone | `docs/workbench/HOT_MEMORY_UNIVERSE.md`; `packages/features_engine/src/features/vix_features.py`; `packages/features_engine/src/hypotheses/vix_modules.py`; Vault `library/11 Options Microstructure.md` | VIX hypotheses depend on `MarketState.cross_asset_features["VIX"]`; coverage and missingness must be visible before interpreting VIX models. | Models API returns total VIX-covered events, cells with VIX, per-row VIX counts, authority source metadata, and fail-closed corruption status when VIX counts are malformed. | Backend tests assert API coverage counts, zero coverage, malformed count rejection, and authority metadata. | implemented; live API verified 2026-06-14 |
| `apps/cockpit/frontend/src/types.ts` | Type additive VIX coverage fields | Same authorities as `apps/cockpit/backend/aggregate/models.py` | UI can only display measured artifact fields; typed fields prevent accidental hidden state. | Add permissive optional fields for model-row and zone-level VIX coverage. | Frontend build. | implemented; frontend build passed 2026-06-14 |
| `apps/cockpit/frontend/src/views/ModelsView.tsx` | Display VIX coverage on Models page | `specs/MODEL_LIFECYCLE.md`; `AGENTS.md`; this checklist Authority Map | Cockpit observes and explains state; it must not fake GREEN or hide missing coverage. | Models page shows VIX coverage summary and per-row VIX event count from API. | Frontend build and browser check if build succeeds. | implemented; browser check passed 2026-06-14 |
| `apps/cockpit/backend/tests/test_cockpit.py` | Guard VIX coverage visibility | `docs/VALIDATION_HONESTY.md`; `tests/test_stage_a/test_stage_a_outputs.py` | Coverage claims require tests/artifacts, not prose. | Add regression tests for VIX coverage projection, zero coverage visibility, malformed count rejection, and authority metadata. | `python -B -m pytest -q apps\cockpit\backend\tests\test_cockpit.py -p no:cacheprovider`. | implemented; targeted pytest passed 2026-06-14 |
| `apps/cockpit/backend/aggregate/options.py` | Add first-class read-only Options zone | `specs/OPTIONS_LANE.md`; Vault `decisions/2026-06-12 Options-lane build decisions (slices 1-7).md`; Vault `sessions/2026-06-13 Options backfill, study verdicts, cockpit integration.md` | CME options research/backtest is allowed in Phases 0-1; live/shadow arm is blocked while the options defect ledger is non-empty; data readiness already lives in data_doctor options checks. | Reuse existing System options readiness and defect-ledger functions; do not add a new pipeline or live controls. | Backend test asserts `/api/options` and `ZONES["options"]` expose independent options lane state, data readiness, defects, and separate research/backtest versus shadow/live status. | implemented; focused backend tests and full cockpit pytest passed 2026-06-14 |
| `apps/cockpit/backend/aggregate/__init__.py`; `apps/cockpit/backend/main.py` | Wire Options zone into cockpit API and all-zone feed | `specs/OPTIONS_LANE.md`; this checklist Options Lane Cockpit Checklist | Owner needs options lane status independent of CME futures status. | Add an `options` zone and `/api/options`; preserve `/api/system` as an infra view. | Backend route and zone-shape tests. | implemented; `/api/options` route test and full cockpit pytest passed 2026-06-14 |
| `apps/cockpit/frontend/src/types.ts`; `apps/cockpit/frontend/src/views/OptionsView.tsx`; `apps/cockpit/frontend/src/App.tsx`; `apps/cockpit/frontend/src/components/Sidebar.tsx` | Add first-class Options cockpit page | Same authorities as `apps/cockpit/backend/aggregate/options.py` | Cockpit must show options data readiness, open defects, research/backtest allowance, shadow/live block status, and no live/paper controls until gates clear. | Add `/options` page with readiness/defect cards sourced only from `/api/options`; no launch or order controls. | Frontend build and browser check. | implemented; frontend build passed 2026-06-14 |
| `apps/cockpit/backend/aggregate/options.py`; `apps/cockpit/frontend/src/types.ts`; `apps/cockpit/frontend/src/views/OptionsView.tsx`; `apps/cockpit/backend/tests/test_cockpit.py` | Opportunity 1: clarify Options lane research/backtest and standalone evidence | `specs/OPTIONS_LANE.md` §1/§3; `packages/hft3/validation/lanes/adapters/cme_options_adapter.py`; `packages/hft3/validation/lanes/registration.py`; `tests/test_workbench/test_options_lane_campaign.py`; `apps/workbench/src/artifacts/paths.py`; `apps/workbench/src/run/campaign_runner.py` | Options Phases 0-1 permit research/backtest. `FOPT_` is the canonical CME options prefix. `OPTIONS_`/`PARITY_`/`DEALER_HEDGING` remain legacy options/parity fixture evidence under the `Lane.EQUITIES` shim. Current `CMEOptionsBacktester` is structural-only. Workbench summaries live under existing `workbench_runs/<campaign>/summary.json` and may be fixture-backed. | Expose `research_backtest_status=allowed`, `execution_status=shadow_live_blocked`, `shadow_live_phase_gate`, `standalone_model_evidence`, and `legacy_options_fixture_evidence`; scan latest existing Workbench summaries without mixing legacy fixture evidence into the FOPT standalone card; classify evidence as structural-only, fixture-only, real-data-backed, or unclassified; render panels distinct from context-feature coverage. | Backend tests assert no `research_only_phase` blocker, structural-only fallback, FOPT-vs-legacy separation, `HFT3_ARTIFACTS_ROOT` discovery, and UI labels. | implemented in Opportunity 1; pytest/build/browser verified 2026-06-14; reviewer subagent final pass found no actionable findings |
| `apps/cockpit/backend/aggregate/options.py`; `apps/cockpit/frontend/src/views/OptionsView.tsx`; `apps/cockpit/backend/tests/test_cockpit.py` | Bad-mood grader fix: harden Options evidence classification | `docs/cockpit/BUILDOUT_CORRECTNESS_CHECKLIST.md` O-008; this checklist Feature Template; Vault `decisions/2026-06-12 Options-lane 2026-embargo policy extension.md`; `packages/hft3/validation/lanes/adapters/cme_options_adapter.py`; `packages/hft3/validation/lanes/lane.py` | Structural adapters must remain `structural_only`, degraded, non-promotable, and not evidence backtests. Real-data evidence needs source IDs, timestamp IDs, nonzero sample accounting, and robustness proof. Any 2026 options usage requires usage-class metadata. Artifact recency must come from semantic run time when available, not arbitrary filesystem mtime. | Make structural/degraded/non-promotable state dominate real-data claims; turn unsupported `real_data_backed: true` into `real_data_claim_unverified`; expose missing proof, promotability, failure notes, trade count, and semantic artifact time in the cockpit; select latest Workbench summary by semantic artifact timestamp before mtime fallback. | Backend tests cover structural dominance, non-promotable dominance, unverified real-data claim rejection, env artifact proof acceptance, and semantic timestamp selection; frontend build and browser/API checks verified the displayed fields and fail-closed live status. | implemented; pytest/build/browser/API verified 2026-06-14 |
| `apps/cockpit/backend/aggregate/options.py`; `apps/cockpit/backend/tests/test_cockpit.py`; `apps/cockpit/frontend/src/views/OptionsView.tsx` | Artifact-backed options-derived context-feature coverage | This checklist Feature Template, CME Options Context Checklist, Options Lane Cockpit Checklist, and Research Measurement Checklist; Vault `Home.md` filtration invariant; Vault `library/System Implications.md`; Vault `library/papers/lee-ryu-yang-yu-2023-options-order-imbalance.md`; Vault `library/papers/odonovan-yu-zhang-2023-option-mm-hedging-liquidity.md`; `specs/FEATURES.md`; `packages/data_layer/packet/schema_research_decision_packet_v1.json` | Options-derived inputs are model features, not informal hints. Lee/Ryu/Yang/Yu show raw options order imbalance is a mixed feature requiring parity-arbitrage separation or proxy-only labeling. O'Donovan/Yu/Zhang justify dealer-gamma-style options features as liquidity-regime conditioning variables. Feature claims require source IDs, timestamp IDs, units, explicit context-ablation rows, and fail-closed missingness. | The Options zone must expose `options_context_features` separately from standalone options strategy evidence; it must read coverage only from existing Workbench artifacts, preserve `not_measured` when absent, and return non-OK proof gaps when a context-feature claim is malformed. Timestamp proof must be validated per row as `source <= feature_available <= target_decision`, without cross-row field mixing. | Backend tests with absent, valid, zero-count, future-target, future-source, mixed-row timestamp, and malformed Workbench summaries; frontend build; `git diff --check`; GraphPost. | implemented; backend pytest, frontend build, diff check, and GraphPost passed 2026-06-14 |
| `apps/workbench/src/run/campaign_runner.py`; `tests/test_workbench/test_options_lane_campaign.py`; `apps/cockpit/backend/aggregate/options.py`; `apps/cockpit/backend/tests/test_cockpit.py` | Producer artifact contract for standalone options/parity fixtures | This checklist Feature Template and Research Measurement Checklist; `docs/project/FEATURE_LITERATURE_TRACEABILITY_MATRIX.md` F004/F005; `specs/OPTIONS_LANE.md`; `docs/VALIDATION_HONESTY.md`; Vault `Home.md` filtration invariant; Vault `library/papers/lee-ryu-yang-yu-2023-options-order-imbalance.md`; Vault `library/papers/odonovan-yu-zhang-2023-option-mm-hedging-liquidity.md` | Standalone options fixture profitability is separate from options-derived context-feature uplift. A target-only versus target-plus-options uplift claim is not valid unless the artifact records PIT source/timestamp proof, units, and explicit context-ablation rows. | Workbench options/parity fixture summaries must emit `context_feature_coverage.status=not_measured` and `context_ablation.status=not_measured` while leaving standalone profitability only in `periods`. Cockpit may display that explicit non-claim as `not_measured` without converting it into an incomplete measured context claim; malformed measured claims still fail closed. | Workbench fixture-summary regression test; cockpit explicit-not-measured contract regression test; contradiction regressions; local GrepLoop; `git diff --check`; GraphPost. | implemented; reviewer, targeted pytest, diff check, and GraphPost passed 2026-06-15 |
...
- [ ] Also allow smaller or adjacent events to act as point-in-time context features for a later target event. Examples: ADP, JOLTS, ECI, durable goods, ISM, EIA, Baker Hughes, construction spending, and similar macro prints.
...
- [ ] The Model Detail event bar chart currently reports per-event expected value segments. It does not prove that one event is being used as a context feature for another later target event.
...
- [ ] Options shadow/live execution is blocked until the options defect ledger is empty and the shadow/live gates are explicitly satisfied; research/backtest remains allowed in Phases 0-1.
...
- [x] Show research/backtest allowed status separately from shadow/live blocked status.
- [x] Show current standalone options model evidence, latest artifact path, and whether artifact is structural-only, fixture-only, or real-data-backed.
- [ ] Show context-feature coverage for options-derived features separately from options-standalone strategy tests.
