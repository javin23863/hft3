...
|---|---|---|---|---|
| Q001 | What exact CME futures/options historical datasets are available for full universe research after the lane split? | F001, F002, F005 | Full model-universe claims require real coverage, not assumed coverage. | Run `python scripts/paid_data_inventory.py --dry-run`; review `runtime/data_audits/paid_data_inventory.json` and `.md` Q001 section for symbols, ranges, futures/options surfaces, manifest schema/path/count validation, and gaps. Use `--verify-q001-hashes` for the slow full-lake SHA256 content pass; default fast mode is fail-closed and reports hashes as format-only. |
