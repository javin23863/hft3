...
| F005 | First-class CME options lane | PARTIALLY_SUPPORTED | Options models are independently testable, not only inputs to futures models. | Vault options microstructure papers: O'Donovan/Yu/Zhang 2023, Lee/Ryu/Yang/Yu 2023, Sahut 2008; `docs/ops/ws0-3-cme-options-fees.md`; `docs/ops/ws0-5-tick-table-status.md`; `specs/OPTIONS_LANE.md`; `packages/hft3/validation/lanes/adapters/cme_options_adapter.py`. | CME options chains, quotes/trades, greeks/underlying, fee/tick rules, expiries, PIT joins. | Separate options model registry, robustness artifacts, lifecycle, and rejection ledger; all-lanes planning emits the structural `FOPT_ES_CALL` CME-options row as `BLOCKED_MISSING_DATA` instead of silently omitting the lane. | Dedicated options lane panel with data readiness, model lifecycle, and blockers. | Options models can run their own research/robustness lifecycle once strict options data and proof artifacts exist. | Reject treating options as only a futures feature source, or counting structural-only FOPT planning rows as real-data-backed options evidence. |
