# DOM_ILLUSION_TRAP — hypothesis spec

status: draft-complete
slug: DOM_ILLUSION_TRAP
kind: hypothesis | hyp_id: 25 | legacy: HYP_25
class: `DOMIllusionTrap` (packages/features_engine/src/hypotheses/modules.py:259)
execution_role: primary_alpha
display: DOM illusion trap

## 1. Market mechanism
Spoof-adjacent size displayed near touch attracts momentum flow; when the displayed size cancels (near-touch cancel pressure) while real flow diverges, traders who leaned on the fake depth are wrong-footed. Their constraint: they sized positions against displayed, not real, liquidity. We trade against the illusion-following flow, paid by their forced re-hedging when the DOM empties.

## 2. Signal formula
```
activation = tanh(3*near_touch_cancel_pressure)
divergence = -agg_imb * near_touch_cancel_pressure
signal     = activation * tanh(2*divergence)                    # modules.py:266-274
```
- Slots: `near_touch_cancel_pressure`, `aggressor_volume_imbalance`.
- Range (-1,1); positive = BUY (cancels plus sell-flow divergence).
...
## Evidence ledger
Pass A (expression v1, flattened semantics): rows=6042, net=-6576.3, realized=-6823.48, win_rate_filled=0.2859. Old-semantics/v1-expression evidence — NOT model-worth evidence.
