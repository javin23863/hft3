# ONE_SIDED_ADD_CANCEL_IMBALANCE — hypothesis spec

status: draft-complete
slug: ONE_SIDED_ADD_CANCEL_IMBALANCE
kind: hypothesis | hyp_id: 15 | legacy: HYP_15
class: `OneSidedAddCancelImbalance` (packages/features_engine/src/hypotheses/modules.py:377)
execution_role: primary_alpha
display: One-sided add/cancel imbalance

## 1. Market mechanism
Liquidity providers add to the side they expect to be paid on and cancel where they fear adverse selection; the aggregate add/cancel skew is the MM cohort's collective directional vote, revealed through quote management rather than trades. Takers who cross against the building side pay. We trade with the add-dominant side.

## 2. Signal formula
```
signal = tanh(log1p(bid_add_cancel_ratio) - log1p(ask_add_cancel_ratio))
                                                                # modules.py:384-392
```
- Slots: `bid_add_cancel_ratio`, `ask_add_cancel_ratio`.
- Range (-1,1); positive = BUY (bid side building).
...
## Evidence ledger
Pass A (expression v1, flattened semantics): rows=8398, net=-6658.98, realized=-8601.08, win_rate_filled=0.3077. Old-semantics/v1-expression evidence — NOT model-worth evidence.
