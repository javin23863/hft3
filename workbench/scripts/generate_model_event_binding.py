#!/usr/bin/env python3
"""Generate workbench/config/model_event_binding.yaml from modules.py AST + PDF overrides."""
...
MODULES = _REPO / "features_engine" / "src" / "hypotheses" / "modules.py"
OUT = _REPO / "workbench" / "config" / "model_event_binding.yaml"
WF = _REPO / "workbench" / "config" / "walk_forward.yaml"
...
def _extract_hyp_bindings() -> dict[str, dict]:
    ...
    for node in tree.body:
        if isinstance(node, ast.ClassDef):
            ...
            mid = f"HYP_{current_hyp}"
            if contexts:
                bindings[mid] = {"required_event_contexts": sorted(contexts)}
            else:
                bindings[mid] = {"default_macro_contexts": list(DEFAULT_MACRO)}
            current_hyp = None

    for hid in range(1, 45):
        mid = f"HYP_{hid}"
        bindings.setdefault(mid, {"default_macro_contexts": list(DEFAULT_MACRO)})
...
def main() -> int:
    ...
    for mid, cfg in hyp.items():
        if "default_macro_contexts" in cfg:
            cfg["default_macro_contexts"] = list(default_macro)

    pdf = {pid: dict(PDF_OVERRIDES.get(pid, {"default_macro_contexts": list(default_macro)})) for pid in (
        f"PDF_MODEL_{i}" for i in range(1, 12)
    )}
