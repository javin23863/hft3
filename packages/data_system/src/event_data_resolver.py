...
Ported from the legacy clone.  Path resolution now goes through
``data_system.src.npz_resolver`` (HFT3_NPZ_ROOT env override with
repo-relative fallback) instead of the retired ``data_roots`` module:
the lake root is the parent of HFT3_NPZ_ROOT (e.g. C:\\hft3-lake) and
each lane lives at <lake>/{npz,mbo_release,sensors}.
"""
...
from data_system.src.npz_resolver import lake_root, npz_root, resolve_npz_for_event
...
def _dedup_dirs(candidates: List[Path]) -> List[Path]:
    dirs: List[Path] = []
    seen: set[str] = set()
...
def npz_search_dirs(repo_root: Path) -> List[Path]:
    """Ordered NPZ dirs: env-resolved lake first, then repo-relative fallback."""
    root = repo_root.resolve()
    return _dedup_dirs([npz_root(root), root / "data" / "npz"])


def sensor_search_dirs(repo_root: Path) -> List[Path]:
    root = repo_root.resolve()
    return _dedup_dirs([lake_root(root) / "sensors", root / "data" / "sensors"])


def mbo_release_search_roots(repo_root: Path) -> List[Path]:
    root = repo_root.resolve()
    return _dedup_dirs([lake_root(root) / "mbo_release", root / "data" / "mbo_release"])
...
def resolve_sensor_for_event(repo_root: Path, event_id: str) -> Tuple[Path, bool]:
    ...
    for d in sensor_search_dirs(repo_root):
        ...
        if hit.is_file() and hit.stat().st_size > 0 and _sensor_parquet_has_finite_level(hit):
            return hit, True
    return lake_root(repo_root) / "sensors" / name, False
