...
def mbo_release_root(repo_root: Path) -> Path:
    """Primary raw-slot root: <lake>/mbo_release when HFT3_NPZ_ROOT is set
    (lake root = parent of HFT3_NPZ_ROOT), else <repo>/data/mbo_release."""
    from data_system.src.npz_resolver import lake_root

    return lake_root(repo_root) / "mbo_release"
