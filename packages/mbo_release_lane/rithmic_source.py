...
def write_release_artifact(
    ...
    *,
    ...
    rep = build_release_event_path(
        ...
        events_ref=(
            str(events_jsonl_path(slot).relative_to(repo_root))
            if events_jsonl_path(slot).is_relative_to(repo_root)
            else str(events_jsonl_path(slot))
        ),
