...
@lru_cache(maxsize=1)
def _regime_boost_labels() -> tuple[frozenset[str], frozenset[str]]:
    path = repo_root() / "packages" / "features_engine" / "config" / "event_context_regime.json"
    if not path.is_file():
        raise FileNotFoundError(
            f"missing {path}; run python tools/economic_event_universe/generate_event_context_labels.py"
        )
    raw = json.loads(path.read_text(encoding="utf-8"))
    return frozenset(raw.get("event_shock", [])), frozenset(raw.get("prop_flatten", []))
...
class RegimeFilter:
    ...
    def update(
        ...
        shock_labels, flatten_labels = _regime_boost_labels()
        if event_context in shock_labels:
            logits["event_shock"] += 2.0
            logits["spread_stress"] += 1.0
        if event_context in flatten_labels:
