...
| **C** | https://github.com/javin23863/hft3/pull/10 | `cursor/autoresearch-pr-c-phases-5-7` | PR-B branch | OPEN — `8a684dd1` Greptile check/body 5/5, but body-prompt follow-up locally fixed and pending push/rerun | Current actionable fixed locally: acceptance `to_dict` identity capture, declared-cert cache normalization, Phase 6 local fixtures; stale outside-diff P1s classified no-op |
...
3. **PR-C (#10) final gate** — current post-`8a684dd1` body-prompt fix batch → push → Greptile last
...
```text
merge-ready: no (post-`8a684dd1` body-prompt fix batch pending push/rerun)
scope-green: yes for PR-C current body-prompt Greptile fix batch
...
pr-greptile-review: PR-B waived-by-owner-20260620; PR-C pending rerun after current body-prompt fix batch
```
