...
| **C** | https://github.com/javin23863/hft3/pull/10 | `cursor/autoresearch-pr-c-phases-5-7` | PR-B branch | OPEN — `adec97dd` Greptile current-head 4/5; quality/ordering fix batch locally verified, pending push/rerun | Current actionable fixed locally: measured low `wf_consistency` rejection, fs_v1 gate-order regression, cockpit no-paired replay `MISSING`, Phase 6 local fixtures |
...
3. **PR-C (#10) final gate** — current quality/ordering Greptile fix batch → push → Greptile last
...
```text
merge-ready: no (`adec97dd` Greptile current-head 4/5; fix batch pending push/rerun)
scope-green: yes for PR-C current quality/ordering Greptile fix batch
...
known-gaps: `adec97dd` Greptile current-head 4/5; quality/ordering fix batch pending push + rerun
pr-greptile-review: PR-B waived-by-owner-20260620; PR-C pending rerun after current quality/ordering Greptile fix batch
```
